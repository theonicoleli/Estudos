...
while True:
    ...
    if 1 <= selecao <= quant_refri:
        ...
        if quantidade_selecionada == 0:
            ...
            continue
        elif quantidade_dos_produtos[selecao-1] > quantidade_selecionada:
            encontrado = False
            for item in continuando_compra:
            ...
            while continuacao_while==0:
                ...
                if pagamento == 1:
                    ...
                    pagamento_pix = 0
                    telefone = int(input('Digite o número de seu telefone (9 digitos): '))
                ...
                if pagamento_total < preco_dos_produtos_atual:
                    ...
                    if pagamento_total == preco_dos_produtos_atual:
                        ...
                        if pagamento_total == 0:
                            ...
                            moedas_cedulas_pagamento = moedas_1_real_pagamento+moedas_25_cent_pagamento+moedas_50_cent_pagamento+cedulas_20_reais_pagamento+cedulas_10_reais_pagamento+cedulas_5_reais_pagamento+cedulas_2_reais_pagamento
                            print(f'Seu troco foi de {pagamento_total}')
                            if moedas_cedulas_pagamento>0:
                                ...
                                print('Estamos sem troco, enviaremos seu dinheiro!')
                                if entrada_moedas_25_cent>0:
                                    print(f'Enviando as moedas de 25 centavos = [{entrada_moedas_25_cent}]')
                                    entrada_moedas_25_cent -= entrada_moedas_25_cent
                                if entrada_moedas_50_cent>0:
        ...
        if 1 <= escolha <= len(lista_de_produtos) and escolha != 0:
            ...
            quantidade_dos_produtos[escolha - 1] += nova_quantidade
            print(f'A nova quantidade de {lista_de_produtos[escolha - 1]} é de: {quantidade_dos_produtos[escolha - 1]}')
        elif escolha == 0:
            ...
            if menu_adm == 1:
                ...
                if escolha_troca in range(len(lista_de_produtos)+1):
                    ...
                    produtos_preco_quantidade[escolha_troca-1][0] = lista_de_produtos[escolha_troca-1]
                    produtos_preco_quantidade[escolha_troca-1][1] = preco_dos_produtos[escolha_troca-1]
                    produtos_preco_quantidade[escolha_troca-1][2] = quantidade_dos_produtos[escolha_troca-1]
                else:
